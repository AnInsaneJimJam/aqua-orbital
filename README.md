...
**Orbital's contracts and first liquidity strategy are active on Arc Testnet.** The user published 10 units of each asset at a 0.05% fee; its activation receipt, live backing and a router quote are [verified](test/evidence/arc-integration/first-strategy.md). Live trader swaps, invoice payments and Privy wallet verification remain unfinished. The persistent local application retains its recorded working flows. See [PROGRESS](PROGRESS.md) for current results and separate mathematical/release obligations.
