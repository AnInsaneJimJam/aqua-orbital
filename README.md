...
**Orbital's contracts, first liquidity strategy, user swap and swap-funded invoice work on Arc Testnet.** The user published 10 units of each asset at a 0.05% fee; [activation and backing](test/evidence/arc-integration/first-strategy.md), the [1-USDC → 0.998491-oUSD6 swap](test/evidence/arc-integration/first-swap.json) and a [0.5-USDC invoice settled from oUSD6](test/evidence/arc-integration/first-payment.json) are receipt-verified. Privy wallet association remains unverified. See [PROGRESS](PROGRESS.md) for local results and separate mathematical/release obligations.
