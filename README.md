...
**Orbital's contracts, first liquidity strategy and first user swap work on Arc Testnet.** The user published 10 units of each asset at a 0.05% fee; [activation and backing](test/evidence/arc-integration/first-strategy.md) and the actual [1-USDC → 0.998491-oUSD6 swap receipt](test/evidence/arc-integration/first-swap.json) are verified. Live invoice payment and Privy wallet association remain unfinished. See [PROGRESS](PROGRESS.md) for local results and separate mathematical/release obligations.
