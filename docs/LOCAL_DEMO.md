...
The [1inch track](https://ethglobal.com/events/ethonline2026/prizes/1inch) expressly permits custom SwapVM instructions, modified SwapVM deployments and local-fork execution demonstrations. Orbital's architecture fits those provisions. The profile below uses a **fresh Anvil chain with deployed upstream Aqua source**, not a fork of an existing public-network deployment. Its real transfer receipts demonstrate local functionality; do not relabel them as fork evidence.

Arc Testnet now has a separate verified **project-deployed upstream AquaRouter** and Orbital graph, activated after twelve user-signed transactions. Use [ARC_DEMO.md](ARC_DEMO.md) for the Arc application on ports **3002/3003**, its public RPC configuration and the two-wallet walkthrough. The local profile below remains on **3000/3001**. Live Arc strategy/swap/invoice evidence and Privy creation/financial-flow verification are still pending; a completed contract deployment does not establish those flows.
...
For Arc, use `pnpm dev:arc` with the supplied public Privy app ID in `apps/web/.env.local` and the public RPC settings in ignored root `.env.arc.local`, as described in [ARC_DEMO.md](ARC_DEMO.md). The runner revalidates all deployment receipts/runtimes through the selected provider before deriving its runtime manifest; it preserves the original activation report. Privy origins/authentication still need dashboard setup and actual wallet verification. Any two distinct user-controlled external wallets can exercise the Arc maker/trader flow; a Privy embedded trader is needed for Privy-specific evidence. The Anvil fixture profile cannot establish Privy sponsor eligibility. Target financial receipts, gas limits and outstanding mathematical/security campaigns remain separate requirements in [PROGRESS](../PROGRESS.md).
