...
`packages/sdk/src/strategy-profile.ts` implements preset construction for 2–8 selected deployment tokens, with integer square roots and directed Q128 coefficients. The public parameter choices remain in `strategyPresets`; presentation receives actual keys/radii and raw allocations from the controller.
...
The selected-token extension makes the single-coin threshold and directed coefficients depend on the selected count `n`; omitted token selections still reconstruct legacy three-token drafts identically. The existing initializer accepted 81 sampled configurations: all three current pairs and baskets of sizes 3–8, each with the three presets at allocations 10, 100 and 1,000,000. Independent Decimal checks agreed at 110/160 digits. Extra assets were synthetic inputs to a pure initializer call, not additions to the Arc allowlist. Local EVM tests also published, traded in both directions and retired every current pair under each preset. See [selected-token evidence and scope](../test/evidence/selected-token-profiles/README.md). These finite checks do not establish universal solver liveness or every configuration in the allocation interval.
