...
The selected registry is **project-deployed upstream AquaRouter**, using unchanged pinned source. [ARC_DEPLOYMENT.md](ARC_DEPLOYMENT.md) lists its verified addresses. The first strategy is active with backed allocations; [strategy evidence](../test/evidence/arc-integration/first-strategy.md), the [first trader swap](../test/evidence/arc-integration/first-swap.json) and the [first swap-funded invoice](../test/evidence/arc-integration/first-payment.json) supersede the earlier empty-route checkpoint. Privy embedded-wallet creation/reconnection and association with financial receipts remain unverified. A chain receipt does not reveal which wallet provider the user selected.
...
**Current checkpoint: funding, strategy publication, the first trader swap and swap-funded invoice are complete.** Trader `0x954ACE1023Cdb3798F69582bbdDcBAcd878B73A9` swapped 1 USDC for 0.998491 oUSD6, then spent 0.5 oUSD6 to settle a 0.5-USDC invoice with a 0.000507-USDC refund. [Payment receipt](../test/evidence/arc-integration/first-payment.json). The existing invoice is paid; create a new unpaid invoice only when trying the updated automatic-input workflow. Wallet-provider verification remains separate. The steps below document the reproducible demonstration, not a request to repeat completed funding/deployment.
...
| Wallet B invoice input | **oUSD6** already received, or funded **oUSD18**; amount calculated automatically, spend only the reviewed amount |

The maker form accepts at least 10 whole units per asset. Select all three tokens for this demonstration; the form defaults to two selected tokens and the Balanced profile has three ticks; the publication review displays the exact rounded amounts and curve configuration. The **40-unit Aqua approval target per asset is an allowance cap**, not a requirement to own or transfer 40 units. Publication needs the actual displayed initial allocation in the maker wallet. Keep additional USDC for gas because that gas spend reduces the same balance backing the strategy.
...
6. Switch to A as merchant and create a fresh **0.5-USDC** invoice, with its own address as the sole 100% recipient. Open the public invoice link, wait for indexed terms, then select the same wallet B used for the swap. Choose **oUSD6** (or funded **oUSD18**); **Amount to pay** calculates automatically. An optional spending limit is available under **Advanced options**. Select **Review payment**, review/sign the exact adapter approval if needed, then separately review and sign the refreshed payment. The default USDC input produces a direct payment; a demo-token input demonstrates swap-funded settlement.
...
The original **20 testnet USDC / nonce 0** [deployer observation](../deployments/5042002/research-2026-09-09.json) predates deployment. Current balances and nonces come from live wallet/RPC reads. Deployment identity, API/indexer readiness, first strategy, trader swap and swap-funded invoice are verified at their recorded checkpoints. Actual Privy creation/reconnection and wallet-provider association with financial receipts remain open. No sponsor approval is implied by deployment.
