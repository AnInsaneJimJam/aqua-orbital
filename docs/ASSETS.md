...
The subsequent animated header/loading mark in `components/OrbitMark.tsx` retains all 41 original SVG path definitions and palette. It groups outer, middle and inner paths separately around the stationary core; the static SVG remains available for Privy branding and the favicon. The larger viewBox allows rotation without clipping. CSS uses the explicit core pivot `(162.829, 157.18)` in SVG user coordinates; percentage origins ignore the negative viewBox offset and caused the corrected wobble. [Current animation checks and retained prototypes](../test/evidence/logo-motion/README.md). Future geometry edits must update the static SVG and shared component together.
