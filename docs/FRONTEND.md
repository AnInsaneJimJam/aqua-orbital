...
All documented routes exist. Public `/pay/[invoiceId]` reads display immutable terms, recipients and direct/swap payment receipts when the configured API supplies checked data. `/swap` now displays validated quote observations and expires stale amounts without requesting a signature. Nineteen Chromium fixture tests pass, including context changes, deployment rotation, cancellation, timeout/retry, large exact amounts and 320px overflow. Strategy publication, invoice actions and swap execution remain guarded previews/unavailable states. Full financial flows, resumed publication, 200% zoom, cross-browser, automated accessibility and performance campaigns are not yet complete. [Invoice UI evidence](../test/evidence/invoice-detail-ui.md), [swap observation evidence](../test/evidence/swap-observation-ui.md).
