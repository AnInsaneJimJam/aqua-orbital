...
Swap/payment review timers start after live preflight and last up to sixty seconds, capped by the transaction/invoice deadline. Submission rechecks the same terms and exact calldata; background repricing pauses during review. Invoice polling and monotonic advancement of a fresh canonical index do not invalidate an open payment review. Terminal/unavailable invoice state, wallet changes, live simulation failure or increased gas budgets still require recovery/review. [Timing repair and focused checks](../test/evidence/quote-refresh/README.md).
...
- `features/usePayment.ts` automatically calculates a read-only input when the payer chooses a token, then orchestrates explicit fresh reviews/signatures. `features/paymentQuote.ts` validates deployment, active-wallet balance and the bounded server observation; `features/paymentLimit.ts` caps read-only search at the current token balance or a lower optional advanced limit. No price is assumed and the search bound is never an approval. `features/Payment.tsx` displays typed formatted state and callbacks within the current invoice template. Context changes invalidate a review permanently; a confirmed payment pauses further calculation while the canonical invoice index catches up. [Focused automatic-input checks](../test/evidence/payment-automatic/README.md).
