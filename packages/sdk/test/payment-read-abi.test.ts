...
test('payment context has a separate four-getter read ABI without altering transaction groups',()=>{
 const abi=reads();assert.ok(Array.isArray(abi),'missing compiled payment read ABI');
 assert.deepEqual(abi.map(entry=>entry.type==='function'?entry.name:null).sort(),['ROUTER','USDC','allowedToken','getInvoice'].sort());
...
test('read selectors and arguments agree with independent cast signatures and compiler provenance',()=>{
 ...
 // Independent `cast sig` observations for these four literal signatures.
 assert.deepEqual(selectors,{getInvoice:'0xcb802c8b',ROUTER:'0x32fe7b26',USDC:'0x89a30271',allowedToken:'0x756742f8'});
