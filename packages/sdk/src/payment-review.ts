...
export function createPaymentDraft(raw:unknown,status:number,manifest:DeploymentManifest,request:PaymentQuoteRequest,nowMs=Date.now()):PaymentDraft {
 ...
 // Authenticate the server's original 20-second quote plan above. The wallet's
 // separately reviewed transaction uses a local 3-minute deadline, capped by
 // the invoice expiry. Amounts, route, recipients and minimum stay unchanged.
 if(input.kind==='swap')input={...input,deadline:context.now+180n<invoice.expiresAt?context.now+180n:invoice.expiresAt};
 buildPaymentTx(context,input);
...
async function liveState(draft:PaymentDraft,port:PaymentPort,now:()=>number){
 ...
 const deadline=draft.input.kind==='swap'?draft.input.deadline:b.expiresAt;
 assertLiveReviewTime(t,deadline,clock);
 ...
 assertLiveReviewTime(t,deadline,now());
 return live;
}
...
export async function preparePaymentReview(draft:PaymentDraft,port:PaymentPort,now:()=>number=Date.now):Promise<PaymentReview>{
 ...
 // Retain source-quote provenance when reconstructing; live checks, not the
 // original context timestamp, establish current transaction validity.
 const ctx=draft.context,plan=stage==='approval'?buildPaymentApprovalTx(ctx,draft.input):buildPaymentTx(ctx,draft.input);
 ...
 const deadline=draft.input.kind==='swap'?draft.input.deadline:draft.input.invoice.expiresAt;
 assertLiveReviewTime(live.block.timestamp,deadline,now());
 const expiresAtMs=reviewExpiresAt(now(),deadline);
...
export async function executePaymentReview(review:PaymentReview,port:PaymentPort,persist:(pending:PendingTransaction)=>void,now:()=>number=Date.now){
 ...
 const {draft,estimate:budget}=review,deadline=draft.input.kind==='swap'?draft.input.deadline:draft.input.invoice.expiresAt;let checked:PaymentBlock|undefined;
 return executeReviewed(review.plan,{...port,send:async(plan,fees)=>{if(!checked||now()>=review.expiresAtMs)throw Error('Payment review expired. Review again.');assertLiveReviewTime(checked.timestamp,deadline,now());return port.send(plan,fees);},estimate:async plan=>{
  const live=await liveState(draft,port,now),stage=live.allowanceRaw<BigInt(draft.observation.data.amountInRaw)?'approval':'payment';
  ...
  await port.canonical(live.block);assertLiveReviewTime(live.block.timestamp,deadline,now());checked=live.block;if(now()>=review.expiresAtMs)throw Error('Payment review expired. Review again.');
  return bounded;
 }},persist);
}
