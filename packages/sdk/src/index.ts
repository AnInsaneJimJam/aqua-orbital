import {concatHex,toHex,isAddress,encodeAbiParameters,keccak256,encodeFunctionData,erc20Abi,parseAbi, type Address,type Hex} from 'viem';
...
function uint(v:bigint,max=MAX){if(v<0n||v>max)throw Error('Amount outside supported range');return v;}
...
export function validateConfig(c:Config){
 ...
 uint(c.makerNonce,(1n<<64n)-1n);feeIn(0n,c.feePpm);
 ...
 if(c.radiiInternal.some(r=>r<10n**12n*(1n<<64n))||c.radiiInternal.reduce((a,b)=>a+b,0n)>=(1n<<160n))throw Error('Radius range');
}
...
export const routerAbi=[{type:'function',name:'swap',stateMutability:'payable',inputs:[{name:'order',type:'tuple',components:orderComponents},{name:'amount',type:'uint256'},{name:'takerTraitsAndData',type:'bytes'}],outputs:[{type:'uint256'},{type:'uint256'},{type:'bytes32'}]},{type:'function',name:'quote',stateMutability:'view',inputs:[{name:'order',type:'tuple',components:orderComponents},{name:'amount',type:'uint256'},{name:'takerTraitsAndData',type:'bytes'}],outputs:[{type:'uint256'},{type:'uint256'},{type:'bytes32'}]}] as const;
...
export const directPaymentAbi=parseAbi(['function payWithUSDC(bytes32 invoiceId)','function createInvoice(uint256 amountDueRaw,uint40 expiresAt,address[] recipients,uint16[] bps,bytes32 memoHash) returns (bytes32)','function cancelInvoice(bytes32 invoiceId)']);
