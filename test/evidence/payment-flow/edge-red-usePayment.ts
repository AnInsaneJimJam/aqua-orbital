...
export type PaymentViewState={phase:Phase;enabled:boolean;connected:boolean;wrongChain:boolean;token:string;tokens:string[];maximum:string;busy:boolean;message?:string;confirmation?:Confirmation;pending?:{hash:string;stage:string;account:string;network:string};
 review?:{stage:'approval'|'payment';input:string;maximum:string;minimum:string;fee:string;refund:string;approval:string;spender:string;network:string;payer:string;gas:string;expires:string;demo:boolean;recipients:{address:string;amount:string}[]};
...
export function usePayment(id:string,eligible:boolean,onReceipt:()=>void):PaymentViewState {
 ...
 const mounted=useRef(true),busy=useRef(false),epoch=useRef(0);
 ...
 async function prepare(){
  ...
  finally{clearTimeout(timer);busy.current=false;if(mounted.current)setActive(false);}
 }
 ...
 const r=fresh?state.review:undefined,d=r?.draft.observation.data;
