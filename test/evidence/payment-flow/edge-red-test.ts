...
test('wallet and chain changes discard the payment review before signing',async({page})=>{
 ...
 await page.evaluate(address=>(window as any).paymentWallet.account(address),f.f.request.payer);await expect(page.getByRole('heading',{name:'Review payment',exact:true})).toHaveCount(0,{timeout:1000});
