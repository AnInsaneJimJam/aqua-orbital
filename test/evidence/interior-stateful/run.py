...
WORK=ROOT/'.cache/interior-stateful'
HERE=Path(__file__).resolve().parent
TEST=CONTRACTS/'test/InteriorStateful.t.sol'
...
def main(stage):
    if stage not in ('smoke','pilot','ci'):raise ValueError('smoke, pilot, or ci')
    runs,depth=(256,128) if stage=='ci' else (32,64)
    ...
    command=['forge','test','--match-contract','^InteriorStatefulInvariantTest$','--match-test','^testDeterministic' if stage=='smoke' else '^invariant_', '--fuzz-seed','0x20260908','--threads','2','--offline','-vv']
