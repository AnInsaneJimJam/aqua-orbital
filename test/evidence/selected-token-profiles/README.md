...
- SDK/web type checking passed. The [production build](https://github.com/AnInsaneJimJam/aqua-orbital/blob/5ab70abc0b313aaed0b665d2e3d0a8a87705d291/test/evidence/selected-token-profiles/build.log) passed using Next 16.3.4 and Node 22.22.1 in `.next-e2e`, separate from the running Arc profile.
...
- The [local EVM lifecycle test](https://github.com/AnInsaneJimJam/aqua-orbital/blob/5ab70abc0b313aaed0b665d2e3d0a8a87705d291/test/evidence/selected-token-profiles/contract.log) passed nine configurations: every two-token subset of the existing three-token fixture under Wide/Balanced/Focused. Each was published, activated, quoted and traded in both directions, retired and docked. Checks cover quote/settlement agreement, fee receipts, selected-only availability and unchanged omitted-token inventory. The reported gas is the aggregate test loop, not a single user transaction.
- All six [browser checks](https://github.com/AnInsaneJimJam/aqua-orbital/blob/5ab70abc0b313aaed0b665d2e3d0a8a87705d291/test/evidence/selected-token-profiles/browser.log) passed. They cover selection/removal with mouse and keyboard, at least-two gating, live pair counts and allocations, an eight-token manifest, responsive widths 1440/390/320, no overflow, selected-pair preparation and reload, legacy draft recovery, and both risk acknowledgements. Browser publication preparation used the existing synthetic wallet/RPC fixture and submitted no transactions.
- The [broader SDK run](https://github.com/AnInsaneJimJam/aqua-orbital/blob/5ab70abc0b313aaed0b665d2e3d0a8a87705d291/test/evidence/selected-token-profiles/sdk.log) passed 17 of 20 test files. Three existing tests (`events`, `payment-read-abi`, `swap-events`) expect two Aqua ABI functions, while the unchanged generated ABI already contains `dock`, `rawBalances` and `ship`. Their only failing assertions are this stale count. The generated ABI and these tests match HEAD; this change does not alter them. There is no configured lint runner; the root lint script reports that type checking is the available static check.
