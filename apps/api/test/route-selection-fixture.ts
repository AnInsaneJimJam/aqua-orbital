...
export function routingFixture(count=6,chainId=31337){
 const configured={...manifest,chainId};
 const configs:Config[]=Array.from({length:count},(_,i)=>({schemaVersion:1,chainId:BigInt(chainId),router:address(11),maker,makerNonce:BigInt(i),tokens:[address(1),address(2),address(3)],decimals:[6,6,18],tickKeys:[3n*GRID/2n,7n*GRID/4n,(1n<<64n)-1n],radiiInternal:[100n,200n,400n].map(r=>r*WHOLE),feePpm:[100,500,1000][i%3]!,initialAmountsRaw:[300_000_000n,300_000_000n,300n*10n**18n]}));
 ...
 const scope=readinessDeploymentScope(configured);
 const snapshot:StrategyReadSnapshot={code:'STRATEGIES_COMPLETE',chainId,deploymentId:scope.id,currentCursor:{height:'3',hash:hash(3)},asOf:{height:'3',hash:hash(3)},indexedAt:new Date().toISOString(),coverage:{fromBlock:'1',toBlock:'3',expectedBlocks:'3',canonicalBlocks:'3',coveredBlocks:'3'},items:records,hasMore:false};
 const input:RoutingInput={manifest:configured,snapshot,intent:{kind:'swap',wallet:address(22),recipient:address(23),tokenIn:address(1),tokenOut:address(3),amountInRaw:'9007199254740993',slippageBps:50,maxCrossings:16},blockTimestamp:'1700000003'};
