...
test('swap starts with USDC to oUSD6, blank input and documented device defaults',async({page})=>{
 const f=await setupSwap(page,{blank:true,dynamic:true});await expect(page.getByRole('combobox',{name:'Input token',exact:true})).toHaveText('USDC');await expect(page.getByRole('combobox',{name:'You receive',exact:true})).toHaveText('oUSD6');await expect(page.getByLabel('You pay',{exact:true})).toHaveValue('');await expect(page.getByRole('button',{name:'Enter an amount',exact:true})).toBeDisabled();
...
test('non-USDC Max uses the full token balance without deducting native gas',async({page})=>{
 const f=await setupSwap(page,{dynamic:true,gasPoor:true});await selectValue(page,'Input token','oUSD18');await expect(page.getByLabel('Input balance',{exact:true})).toHaveText('Balance: 0.009007199254740993 oUSD18');await page.getByRole('button',{name:'Max',exact:true}).click();await expect(page.getByLabel('You pay',{exact:true})).toHaveValue('0.009007199254740993');expect(f.sent.size).toBe(0);
});
