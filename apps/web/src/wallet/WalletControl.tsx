...
export default function WalletControl(){
 ...
 if(!wallet.connected)return <div><button className="button secondary compact" disabled={!wallet.ready&&!slow} onClick={slow&&!wallet.ready?()=>window.location.reload():wallet.connect}>{wallet.ready?'Connect wallet':slow?'Retry wallet connection':'Loading wallet…'}</button>{slow&&!wallet.ready&&<p className="hint" role="status">Wallet setup is taking longer than expected. Retry when your connection is ready.</p>}</div>;
 return <Dialog.Root open={open} onOpenChange={setOpen}><Dialog.Trigger asChild><button className="button secondary compact" aria-label="Manage connected wallet">{wallet.address?.slice(0,6)}…{wallet.address?.slice(-4)}</button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay className={styles.overlay}/><Dialog.Content className={styles.content}><Dialog.Title>Connected wallet</Dialog.Title><Dialog.Description>Select the wallet to use for your next review.</Dialog.Description>
  ...
  <Link className="button secondary" href="/fund" onClick={()=>setOpen(false)}>Get demo tokens</Link><div className={styles.actions}><button className="button secondary" onClick={()=>{wallet.connect();setOpen(false);}}>Connect another</button><button className="button secondary" onClick={()=>{wallet.disconnect();setOpen(false);}}>Disconnect</button></div>
