...
export function StrategiesView({state,cards}:{state:StrategiesViewState;cards?:ReactNode}){
 return <section className={`page ${styles.page}`}>
  ...
  </div><aside className={styles.create}><h2>New strategy</h2><p>Concentrated liquidity, held in your wallet.</p><ul className={styles.createAssets}>{['USDC','oUSD6','oUSD18'].map(symbol=><li key={symbol}><TokenIcon symbol={symbol} size={32}/><strong>{symbol}</strong></li>)}</ul><p className="hint">Choose an allocation, concentration profile and swap fee. Review each publication step before signing.</p><Link className="button full" href="/liquidity/new">Create strategy <span aria-hidden="true">+</span></Link></aside></div>
 </section>;
}
