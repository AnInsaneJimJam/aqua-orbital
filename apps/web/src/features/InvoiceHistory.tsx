...
export function InvoiceHistory(){
 ...
 {s.error?<p className="notice" role="status">Invoice history is unavailable. Refresh to restart the canonical listing.</p>:s.loading?<p className={styles.empty} role="status">Reading invoice history…</p>:!s.items?.length?<p className={styles.empty}>No invoices found on this page.</p>:s.items.map(i=><article className={`panel ${styles.invoice}`} key={i.id}><div className={styles.invoiceHeader}><h3>{i.amount}</h3><span className="pill">{i.status}</span></div><p>{i.recipients} settlement recipient{i.recipients===1?'':'s'} · Expires {i.expiry}</p><Link className="inline-link" href={`/pay/${i.id}`}>Open invoice <span aria-hidden="true">→</span></Link></article>)}
