...
export default function Proof(){
 ...
 return <section className={`page ${styles.page}`}><div className="page-head"><div><div className="eyebrow">Behind Orbital</div><h1>Protocol notes</h1><p>Inspect the mathematics, execution, and receipts behind the application.</p></div></div>
  ...
  <section className={styles.section} aria-labelledby="integration-checks"><div className={styles.sectionHeading}><div><h2 id="integration-checks">Integration evidence</h2><p>Verified Arc identity, Privy wallet integration, and retained financial-flow checks.</p></div></div>
  ...
  <section className={styles.section} aria-labelledby="source-checks"><div className={styles.sectionHeading}><div><h2 id="source-checks">Source &amp; local checks</h2><p>Read the implementation alongside its tests and retained results.</p></div><EvidenceLink href={repositoryUrl}>Open repository</EvidenceLink></div>
   ...
   <div className={styles.sourceFoot}><p className={styles.note}>Local test results apply to their recorded inputs and source versions. Repository access may be required.</p><EvidenceLink href={repositoryFile('test/evidence/INDEX.md')}>Full evidence index</EvidenceLink></div>
  </section>
  ...
  <aside className={styles.limits} aria-labelledby="remaining-acceptance"><h2 id="remaining-acceptance">What remains open</h2><p>Record the Privy wallet-provider association for the demonstrated financial receipts. Full mathematical and release campaigns and an independent security audit also remain open; the verified Arc deployment and executed financial flows are recorded above.</p><EvidenceLink href={repositoryFile('docs/audits/RELEASE_GAP_REVIEW.md')}>Read the release gap review</EvidenceLink></aside>
 </section>;
}
