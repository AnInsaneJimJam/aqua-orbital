...
export function StrategyAdminView({state:s}:{state:StrategyAdminViewState}){
 ...
 return <section aria-label="Strategy administration" className={styles.observation}>
  <h3>Manage your strategy</h3><p>Deactivation first retires the Orbital strategy, then docks its Aqua allocation. Each step needs its own review and wallet confirmation. Tokens are already in your wallet.</p>
  ...
  <div className={styles.actions}>{s.pending?<button className="button" disabled={s.busy} onClick={s.resume}>Resume strategy receipt</button>
   ...
   :<><button className="button" disabled={s.busy||!s.enabled} onClick={()=>s.prepare({kind:'deactivate'})}>{s.busy?'Preparing review…':'Review deactivation step'}</button>
