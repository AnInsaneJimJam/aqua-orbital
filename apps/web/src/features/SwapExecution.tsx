...
export function SwapExecution({state:s,network}:{state:SwapExecutionView;network:string}){
 ...
 return <section aria-label="Swap execution">
  {r&&<><h2>{r.stage==='approval'?'Review token approval':'Review swap'}</h2><dl className="hint">
  ...
  {c&&<div aria-label="Swap receipt"><h2>{c.reverted?'Transaction reverted':c.approval?'Approval confirmed':'Swap complete'}</h2>
   <dl className="hint">{c.input&&<><dt>Actual input</dt><dd className="mono">{c.input}</dd><dt>Actual output</dt><dd className="mono">{c.output}</dd><dt>Actual fee</dt><dd className="mono">{c.fee}</dd></>}<dt>Gas paid</dt><dd>{c.gas}</dd></dl>
