...
export function SwapExecution({state:s,network}:{state:SwapExecutionView;network:string}){
 ...
 return <section aria-label="Swap execution" className={styles.execution}>
  {r&&<><h2>{r.stage==='approval'?'Review token approval':'Review swap'}</h2><p className="notice">This is a testnet transaction. Demo tokens have no redemption value.</p><dl className={styles.facts}>
   <dt>Network</dt><dd>{network}</dd><dt>You pay</dt><dd>{r.input}</dd><dt>Minimum received</dt><dd>{r.minimum}</dd>
   <dt>Trading fee · included</dt><dd>{r.fee}</dd><dt>Recipient</dt><dd className="mono">{r.recipient}</dd>
  ...
  {s.pending&&<><p className="notice">Swap {s.pending.stage} submitted.</p><TransactionLink hash={s.pending.hash}/><p className="mono">{s.pending.hash}</p><p className="hint">Payer: <span className="mono">{s.pending.account}</span></p><button className="button full" disabled={s.busy} onClick={s.resume}>Resume swap receipt</button></>}
  {c&&<div aria-label="Swap receipt"><h2>{c.reverted?'Transaction reverted':c.approval?'Approval confirmed':'Swap complete'}</h2>
   <dl className={styles.facts}>{c.input&&<><dt>Actual input</dt><dd>{c.input}</dd><dt>Actual output</dt><dd>{c.output}</dd><dt>Actual fee</dt><dd>{c.fee}</dd></>}<dt>Gas paid</dt><dd>{c.gas}</dd></dl>
   <TransactionLink hash={c.hash} label="Open transaction in explorer"/>
