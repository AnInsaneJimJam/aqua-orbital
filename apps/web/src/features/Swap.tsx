...
export function SwapView(state:SwapViewState){
 ...
 <div className="field"><label htmlFor="output-token">You receive</label><select id="output-token" value={output} onChange={e=>setOutput(e.target.value)}>{symbols.filter(s=>s!==input).map(s=><option key={s}>{s}</option>)}</select>{state.quoteView?<output className="mono" aria-label="Quoted output">{state.quoteView.output}</output>:<span className="hint">Output appears after a certified onchain quote.</span>}</div>
 ...
 {state.quoteView&&<section aria-label="Quote observation" className="hint">
 ...
 {state.expired&&<p role="status" className="notice">{copy.swap.expired}</p>}
