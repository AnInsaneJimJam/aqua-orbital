...
export function InvoiceAdminView({state:s}:{state:InvoiceAdminViewState}){
 ...
 return <div className={styles.payment} aria-label={s.kind==='create'?'Invoice creation':'Invoice cancellation'}>
  {r&&<div aria-label="Invoice action review"><h3>{s.kind==='create'?'Review invoice creation':'Review invoice cancellation'}</h3>
  ...
  {s.pending&&<div className="notice"><p>Submitted invoice action</p><p className="mono">{s.pending.hash}</p><p>Merchant: <span className="mono">{s.pending.account}</span></p></div>}
