...
export default function Payments(){
 ...
 return <section className="page"><div className="page-head"><div><h1>Payments</h1><p>Create an invoice. Get paid in USDC.</p></div></div>
  <div className={styles.workspace}><div className="panel"><h2>Create an invoice</h2>
   ...
   <details><summary>Recipients &amp; splits</summary>{recipients.map((r,i)=><div key={i} className="stack"><div className="field"><label htmlFor={`recipient-${i}`}>Recipient {i+1}</label><input id={`recipient-${i}`} value={r.address||(i===0?wallet.address??'':'')} maxLength={42} disabled={locked} onChange={e=>changeRecipient(i,'address',e.target.value)} placeholder={i===0?'Connect the merchant wallet':'0x…'}/></div>
