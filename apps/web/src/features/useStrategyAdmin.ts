...
export function useStrategyAdmin(id:string,input?:StrategyInput,onReceipt?:()=>void):StrategyAdminViewState{
 ...
 function finish(pending:PendingStrategyAdmin,receipt:TransactionReceipt){
  ...
  if(mounted.current){setState({context:liveContext.current,phase:receipt.status==='success'?'confirmed':'reverted',confirmation,message:(receipt.status==='reverted'?'Transaction reverted. Review current strategy state before retrying.':pending.kind==='ship'?'Aqua allocation published. Review activation to make the strategy tradable.':pending.kind==='activate'?'Orbital strategy activated. It can now receive trades.':pending.kind==='retire'?'Strategy retirement confirmed. Review the next step to dock its Aqua allocation.':pending.kind==='dock'?'Aqua docking confirmed. Your tokens remain in your wallet.':pending.reset?'Allowance reset confirmed. Review the token again to set its bounded cap.':'Bounded Aqua approval confirmed. A new strategy still needs publication and activation.')+storage});onReceipt?.();}
 }
 ...
 return {phase:state.pending?state.phase:fresh?state.phase:'stale',owner,enabled,wrongChain,busy:active,message:state.message,confirmation:state.confirmation,
  ...
  review:r?{kind:r.kind,title:r.kind==='ship'?'Publish Aqua allocation':r.kind==='activate'?'Activate Orbital strategy':r.kind==='retire'?'Retire strategy':r.kind==='dock'?'Dock Aqua allocation':r.reset?`Reset ${token?.symbol??'token'} Aqua allowance`:`Approve ${token?.symbol??'token'} for Aqua`,maker:r.plan.account,network:selectedChain.name,target:r.plan.to,orderHash:hashOrder(r.input.order),
