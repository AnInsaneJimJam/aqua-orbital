...
export function useStrategyAdmin(id:string,input?:StrategyInput,onReceipt?:()=>void):StrategyAdminViewState{
 ...
 function finish(pending:PendingStrategyAdmin,receipt:TransactionReceipt){
  ...
  if(mounted.current){setState({context:liveContext.current,phase:receipt.status==='success'?'confirmed':'reverted',confirmation,message:(receipt.status==='reverted'?'Transaction reverted. Review current strategy state before retrying.':pending.kind==='ship'?'Aqua allocation published. Review activation to make the strategy tradable.':pending.kind==='activate'?'Orbital strategy activated. It can now receive trades.':pending.kind==='retire'?'Strategy retirement confirmed. Review the next step to dock its Aqua allocation.':pending.kind==='dock'?'Aqua docking confirmed. Your tokens remain in your wallet.':pending.reset?'Allowance reset confirmed. Review the token again to set its bounded cap.':'Bounded Aqua approval confirmed.')+storage});onReceipt?.();}
 }
