"""Freeze authored inputs around SDK and complete frontend swap control verification.
...
out = root / 'test/evidence/swap-controls'
...
def inputs():
    ...
    return [{'path':name,'sha256':digest(root/name)} for name in sorted(names) if name and name!='apps/web/next-env.d.ts' and (
        ...
        or name in {'package.json','pnpm-lock.yaml','pnpm-workspace.yaml','tsconfig.base.json','scripts/audit-payment-ui.py','scripts/audit-swap-ui.py','scripts/audit-swap-controls.py','scripts/recorded_command.py'})]
