"""Freeze authored inputs around SDK and complete frontend payment verification.
...
out = root / 'test/evidence/payment-flow'
...
def inputs():
    ...
    return [{'path':name,'sha256':digest(root/name)} for name in sorted(names) if name and name!='apps/web/next-env.d.ts' and (
        ...
        or name in {'package.json','pnpm-lock.yaml','pnpm-workspace.yaml','tsconfig.base.json','scripts/audit-payment-ui.py','scripts/recorded_command.py'})]
