...
try{
 ...
 if(action==='plan'){
  ...
  const {report,directory}=await runLocalDeployment();console.log(JSON.stringify({status:report.status,verified:false,chainStopped:report.chainStopped,contracts:report.contracts.length,receipts:report.receipts.length,evidence:directory},null,2));
 }
