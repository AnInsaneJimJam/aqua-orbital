...
paths=sorted((root/'packages/reference').rglob('*.py'))+sorted((root/'packages/reference/fixtures').glob('*.json'))+sorted((root/'packages/contracts/test/fixtures').glob('*.sol'))+[root/f'packages/contracts/test/{name}.t.sol' for name in ['RootBracket','InteriorSwap','InteriorExecution','FrontierTurn','FrontierEndpoint','FrontierEvents','FrontierComposition','SeedProposal','OutwardRetention','FrontierPriceExclusion','MixedExecution','ReachableComposition','PilotReversal']]+[Path(__file__).resolve(),root/'packages/reference/requirements.txt',root/'docs/MATH.md',root/'docs/NUMERICS.md']
...
manifest={
 ...
 'mathematics':{
  'assertion_tested':'Explicit per-tick primal/dual feasibility on named fixtures; analytic sphere/cap/initialization benchmarks; both crossing roots, bounded crossing schedules, GRID inward releases and fixed-frontier paths; scalar/support certificates; identified fixed-input endpoint raw-output/shortfall goldens; precision stability; retained failures of unconditional rounding/path simplifications; retained 128-action mixed-pilot consistency and actual endpoints; exact pair-difference/bisection proposal oracle.',
  'coefficient_domain':'mpmath arbitrary-precision real numerical arithmetic, generally 110 digits; selected checks repeated at 160 digits; Fraction/isqrt exact rational zero-price witnesses; Fraction and integer bisection for the lower-sheet oracle',
  ...
  'inputs':inputs,
  'bounds':{'dimensions':[2,3,4,5,6,7,8,16,32],'onchain_profile_max_dimension':8,'flagship_directed_pairs':6,'fixed_input_endpoint_dimensions':[2,3,8],'exact_zero_price_dimension':6,'dual_newton_maxsteps':150,'dual_initial_guesses':[0,-0.5,0.5,-2],'timeout_seconds':180,'precision_stability_absolute_relative_threshold':'1e-90','lower_sheet_exact_cases':47,'lower_sheet_small_exhaustive_cases':625,'scope':'Dimension sweep covers per-tick feasibility only. Retained mixed pilot has 32 configurations, n2/n3/n5/n8, 1/2/3/8 ticks and four net-input actual-payout steps; this runner checks its consistency and per-tick actual endpoints, not full regeneration or production execution. Separate mixed-pilot manifests record generation and linked observations. Exact lower-sheet proposal cases cover n2..8 without a root/trade claim.'},
