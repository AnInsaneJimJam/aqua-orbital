...
async function existingSuiteEvidence(mode){
 const report=await record(`test/evidence/engine-suite/${mode}-summary.json`);
 const manifest=await record(`test/evidence/engine-suite/${mode}/manifest.json`);
 const verified=report?.status==='passed'&&report.sourceStable===true&&report.counts?.passed>0
  &&report.counts.failed===0&&report.counts.skipped===0
  &&manifest?.run?.status==='completed'&&manifest.run.exit_status===0
  &&Array.isArray(manifest.input_artifacts)&&manifest.input_artifacts.length>0
  &&manifest.input_artifacts.every(input=>input.sha256===input.sha256_after)
  &&Array.isArray(manifest.outputs)&&manifest.outputs.length>0
  &&await matchingRecordedInputs({inputs:[...manifest.input_artifacts,...manifest.outputs]});
 return {report,verified};
}
const [completeContracts,completeReference]=await Promise.all(['contracts','reference'].map(existingSuiteEvidence));
const completeSuiteVerified=completeContracts.verified&&completeReference.verified;
...
const items=[
 ...
 {id:'reference',label:'Independent reference fixtures',status:completeReference.verified||referenceCount>0&&/^OK\s*$/m.test(reference)?'verified':'unavailable',detail:completeReference.verified?`${completeReference.report.counts.passed} tests passed in the complete existing reference suite with unchanged inputs and matching output hashes. Existing exact and high-precision checks are finite evidence, not a proof of the production solver.`:`${referenceCount} recorded bounded numerical regressions; selected 110/160-digit checks. This is not a proof of the production solver.`},
 {id:'contracts',label:'Local contract tests',status:completeContracts.verified||contractCount>0&&contractRun?.[2]==='0'&&contractRun?.[3]==='0'?'verified':'unavailable',detail:completeContracts.verified?`${completeContracts.report.counts.passed} tests across ${completeContracts.report.counts.suites} suites passed in the complete existing default-profile run, including arithmetic, curve execution, Aqua settlement, payments and existing fuzz/invariant cases. Input and output hashes match. Full release campaigns remain outstanding.`:`${contractCount} recorded tests, including real six-pair interior Aqua swaps, an initialized mixed outward/reverse sequence, an invoice and local security/cycle checks. Separate receipt probes and frozen campaigns retain their source boundaries. Full release campaigns remain outstanding.`},
