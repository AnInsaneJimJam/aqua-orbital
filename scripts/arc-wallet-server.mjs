...
async function status(){
 ...
 if(!['project-deployed-upstream','existing-authenticated'].includes(value.aquaDeployment)||!Number.isSafeInteger(value.total)||value.total<1||!Number.isSafeInteger(value.confirmed)||value.confirmed<0||value.confirmed>value.total)throw Error('The prepared plan has invalid deployment provenance or progress');
 return value;
}
...
<section class="card" aria-labelledby="status-heading"><h2 id="status-heading">Deployment status</h2><dl><dt>Network</dt><dd>Arc Testnet · 5042002</dd><dt>Authorized wallet</dt><dd><code id="deployer">Loading…</code></dd><dt>Aqua provenance</dt><dd id="provenance">Loading…</dd><dt>Plan</dt><dd id="plan">—</dd><dt>Progress</dt><dd id="progress">—</dd><dt>Connected wallet</dt><dd id="wallet">Not connected</dd></dl><p class="muted" id="provenance-description"></p><p class="muted" id="ownership"></p><ul class="warning" id="blockers" hidden></ul><div class="actions"><button id="connect" type="button">Connect browser wallet</button><button class="secondary" id="refresh" type="button">Refresh status</button></div></section>
...
<section class="card" id="review" hidden><h2 id="next-label">Next transaction</h2><dl><dt>Action</dt><dd id="action">—</dd><dt>Recipient</dt><dd id="recipient">—</dd><dt id="created-address-label" hidden>Predicted address</dt><dd id="created-address" hidden>—</dd><dt>USDC sent</dt><dd>0 · deployment gas is charged separately</dd><dt>This step's gas cap</dt><dd id="gas">—</dd><dt>Nonce</dt><dd id="nonce">—</dd></dl><details><summary>Exact unsigned transaction</summary><pre id="transaction"></pre></details><p class="warning">Confirm the authorized wallet, network, action and this step's gas budget before opening your wallet. Each transaction needs its own wallet approval.</p><button id="send" type="button" disabled>Review and sign in wallet</button></section>
...
function render(){
 ...
 $('deployer').textContent=deployment.deployer;$('plan').textContent=deployment.planId;$('progress').textContent=deployment.confirmed+' of '+deployment.total+' transactions confirmed · '+(deployment.total-deployment.confirmed)+' remaining · '+deployment.phase;
 const projectAqua=deployment.aquaDeployment==='project-deployed-upstream';
 $('provenance').textContent=projectAqua?'Project-deployed upstream Aqua':'Existing Aqua (authentication required)';
 $('provenance-description').textContent=projectAqua?'This plan deploys the unchanged upstream AquaRouter contract for Orbital. Its upstream source is verified; this project deployment is not an official 1inch deployment or endorsement.':'This mode requires reviewed provenance for an existing Aqua deployment. Any unresolved identity or deployment checks are listed below.';
 $('ownership').textContent=(projectAqua?'The AquaRouter helper owner remains your authorized wallet. ':'')+'The custom Orbital router is separate: the final transaction renounces its ownership before strategies can be activated.';
 ...
 if(deployment.next){const tx=deployment.next.transaction;$('next-label').textContent='Step '+(deployment.confirmed+1)+' of '+deployment.total+': '+deployment.next.label;$('action').textContent=tx.to?'Contract call':'Create contract';$('recipient').textContent=tx.to||'Contract creation';const predicted=!tx.to&&typeof deployment.next.address==='string'&&/^0x[0-9a-fA-F]{40}$/.test(deployment.next.address);$('created-address-label').hidden=!predicted;$('created-address').hidden=!predicted;$('created-address').textContent=predicted?deployment.next.address:'—';$('gas').textContent=String(deployment.next.gasBudgetUsdc)+' USDC';$('nonce').textContent=/^0x[0-9a-fA-F]+$/.test(tx.nonce)?BigInt(tx.nonce).toString():'Invalid';$('transaction').textContent=JSON.stringify(tx,null,2);}
