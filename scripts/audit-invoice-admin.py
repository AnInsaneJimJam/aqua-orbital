"""Freeze authored inputs around invoice administration and full app regressions.
...
out = root / 'test/evidence/invoice-admin/final'
...
def inputs():
    ...
    return [{'path':name,'sha256':digest(root/name)} for name in sorted(names) if name and name!='apps/web/next-env.d.ts' and (
        ...
        or name in {'package.json','pnpm-lock.yaml','pnpm-workspace.yaml','tsconfig.base.json','scripts/audit-payment-ui.py','scripts/audit-swap-ui.py','scripts/audit-swap-controls.py','scripts/audit-invoice-admin.py','scripts/recorded_command.py','scripts/evidence.mjs'})]
...
record={'schemaVersion':1,'capturedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'parentCommit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),
        'scope':'SDK/API/shared and workspace types; complete Chromium fixture suite; production compilation. No real wallet or financial receipt verification.',
...
for label,command in commands:
    if label=='evidence':
        ...
        for source,target,archive in [('sdk','sdk-green.txt','sdk-checkpoint-114.txt'),('api','api.txt','api-checkpoint-210-pre-admin.txt'),('browser','browser.txt','browser-checkpoint-56.txt')]:
