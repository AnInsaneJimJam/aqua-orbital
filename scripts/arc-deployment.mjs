...
async function main() {
  ...
  if (positionals.length !== 1 || !['inspect', 'prepare', 'status', 'record', 'activate', 'verify-active'].includes(command)) {
    throw Error('Usage: pnpm deploy:arc inspect|prepare --deployer ADDRESS [--self-deploy-aqua | --aqua-evidence FILE] [--plan FILE]; status|record|activate --plan FILE [--hash HASH]; verify-active [--rpc-url PUBLIC_HTTPS_URL]. Browser signing: pnpm deploy:arc:wallet --plan FILE.');
  }
  ...
  if (values['rpc-url'] && command !== 'verify-active') throw Error('--rpc-url selects a public read provider only for read-only verify-active.');
  ...
  else result = await verifyActiveArcDeployment({rpcUrl: values['rpc-url']});
