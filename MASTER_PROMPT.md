...
**Tests first:** write the extensive acceptance suite for a milestone before its production implementation. Establish meaningful failing behavior against compiling interfaces/stubs, then implement and make it pass. Do not begin with a polished mock frontend and substitute it for the protocol. Do not implement a two-token constant-product AMM and call it Orbital.
...
| `GET /makers/:address/strategies` | Paginated wallet-owned strategies including incomplete/docked/retired states |
...
Use PostgreSQL `numeric(78,0)` for bounded uint256 values; wide squared aggregates are validated decimal text/JSON strings. No double-precision money. Materialize an entire block and advance cursor in one database transaction. Restart is idempotent. Fetch from recorded deployment blocks, polling every 2 seconds in chunks up to 500 blocks; reduce chunk size on provider log limits.
...
Implement named reusable components: `AppHeader`, `WalletButton`, `NetworkBanner`, `TokenBadge`, `TokenAmountInput`, `TokenPickerDialog`, `AmountBreakdown`, `TransactionStepper`, `InlineError`, `EmptyState`, `StrategyCard`, `TickProfilePreview`, `InventoryCoverageTable`, `InvoiceSummary`, and `EvidenceRow`. Their variants derive from the token system; do not duplicate wallet, quote or transaction logic per page.

All inputs have persistent visible labels, localized human formatting with exact raw values available on demand, and field-level validation. Accept decimal input through a decimal-string parser; reject scientific notation, negatives, more fractional digits than token decimals, and overflow. Preserve the user's editing string; format on blur, never move the caret on every keystroke. Copy-address buttons copy the full checksummed address, not the truncated display. Token selectors show name, symbol, network, balance, mock badge, and address suffix.
...
Debounce amount edits 300ms. Query only for positive valid input, distinct tokens and enabled chain. Older in-flight responses cannot overwrite a newer pair/amount/account request. Refresh a still-visible quote every 10 seconds while the tab is active; expire visually after 20 seconds. Amounts do not animate when they change. Show how many strategies were compared and the selected maker; phrase “Best among the strategies checked,” not “Best price across DeFi.”
...
**Assets:** fixed three demo assets with balance, decimals, mock labels, and allowance summary. Field `Allocate per asset` default 10 whole units. Show total nominal allocation and USDC gas reserve. Initial all-equal amounts are a deliberate strategy starting condition; no single-token deposit simulation. Insufficient actual balances link to Circle's testnet faucet for USDC and the app's test-only mock mint flow for demo tokens. Faucet clicks are user actions, never a secret backend transfer.
...
Show merchant address, amount due in USDC, expiry, status, and recipient split. Wallet connected payer selects USDC/oUSD6/oUSD18. For non-USDC input, display estimated input, maximum input, Orbital fee, USDC expected output, due amount and excess refund. The input ceiling is explicit; the browser never authorizes an unbounded invoice search. Approve the adapter for the reviewed exact maximum needed transaction amount, then re-quote and present one `Pay invoice` action.
